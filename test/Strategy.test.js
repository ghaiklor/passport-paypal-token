...
var PaypalTokenStrategy = require('../');
var fakeProfile = JSON.stringify(require('./fixtures/profile.json'));

describe('PaypalTokenStrategy:init', function () {
  it('Should properly export Strategy constructor', function () {
    assert.equal(typeof PaypalTokenStrategy, 'function');
    assert.equal(typeof PaypalTokenStrategy.Strategy, 'function');
    assert.equal(PaypalTokenStrategy, PaypalTokenStrategy.Strategy);
  });

  it('Should properly initialize', function () {
    var strategy = new PaypalTokenStrategy({
      clientID: '123',
      clientSecret: '123'
    }, function () {
    });
...
describe('PaypalTokenStrategy:authenticate', function () {
  describe('Authenticate without passReqToCallback', function () {
    var strategy;

    before(function () {
      strategy = new PaypalTokenStrategy({
        clientID: '123',
        clientSecret: '123'
...
describe('PaypalTokenStrategy:userProfile', function () {
