...
import PaypalTokenStrategy from '../../src/index';
...
describe('PaypalTokenStrategy:init', () => {
  it('Should properly export Strategy constructor', () => {
    assert.isFunction(PaypalTokenStrategy);
  });

  it('Should properly initialize', () => {
    let strategy = new PaypalTokenStrategy(STRATEGY_CONFIG, BLANK_FUNCTION);

    assert.equal(strategy.name, 'paypal-token');
  });

  it('Should properly throw error on empty options', () => {
    assert.throws(() => new PaypalTokenStrategy(), Error);
  });
});

describe('PaypalTokenStrategy:authenticate', () => {
  describe('Authenticate without passReqToCallback', () => {
    let strategy;

    before(() => {
      strategy = new PaypalTokenStrategy(STRATEGY_CONFIG, (accessToken, refreshToken, profile, next) => {
  ...
  describe('Authenticate with passReqToCallback', () => {
    ...
    before(() => {
      strategy = new PaypalTokenStrategy(Object.assign(STRATEGY_CONFIG, {passReqToCallback: true}), (req, accessToken, refreshToken, profile, next) => {
...
describe('PaypalTokenStrategy:userProfile', () => {
